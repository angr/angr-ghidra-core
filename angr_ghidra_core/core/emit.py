...
UNDEF8_ID = BUILTIN_ID_HEADER | 0x22  # matches the coretypes id we register
...
class ResponseEmitter:
    ...
    def emit_doc(self, name: str, entry: int, size: int, codegen) -> bytes:
        enc = PackedEncoder()
        enc.open_element(ids.ELEM_DOC)
        self._emit_model_function(enc, name, entry, size)
        self._emit_markup_function(enc, codegen)
        enc.close_element(ids.ELEM_DOC)
        return enc.to_bytes()

    # ---- model <function> (HighFunction.decode-compatible, no ast/highlist) ----

    def _emit_model_function(self, enc: PackedEncoder, name: str, entry: int, size: int) -> None:
        ...
        enc.write_string(ids.ATTRIB_NAME, "undefined8")
        enc.write_unsigned(ids.ATTRIB_ID, UNDEF8_ID)
    ...
    def _emit_markup_function(self, enc: PackedEncoder, codegen) -> None:
        enc.open_element(ids.ELEM_FUNCTION)
        ...
        for text, obj in codegen.cfunc.c_repr_chunks(indent=0):
            ...
            for i, seg in enumerate(segments):
                if i > 0:
                    # newline boundary: measure indent of the following segment
                    stripped = seg.lstrip(" ")
                ...
                if not seg:
                    continue
                self._emit_token(enc, seg, color)
        enc.close_element(ids.ELEM_FUNCTION)

    def _emit_token(self, enc: PackedEncoder, text: str, color: int | None) -> None:
        if color is None:
            enc.open_element(ids.ELEM_SYNTAX)
            enc.write_string(ids.ATTRIB_CONTENT, text)
            enc.close_element(ids.ELEM_SYNTAX)
        elif color == VARIABLE_COLOR:
